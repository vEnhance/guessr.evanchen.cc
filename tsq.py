...
for letter in string.ascii_uppercase:
    fn_names[f"-{letter}+2*foot"] = 3
    fn_names[f"-{letter}p+2*foot"] = 3
    fn_names[f"-{letter}s+2*foot"] = 3
    for digit in "0123456789":
        fn_names[f"-{letter}_{digit}+2*foot"] = 3
...
if opts.preamble:
    print(f"defaultpen(fontsize({opts.fontsize}));")
...
for line in lines:
    ...
    if "=" in line:
        ...
        if point_name != "":
            print(f"pair {point_name} = {expr};")
        if draw_point:
            if label_point:
                dot_code += f'dot("${label_name}$", {point_name}, {direction});\n'
            else:
                dot_code += f"dot({point_name if point_name else expr});\n"
